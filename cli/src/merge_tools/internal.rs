...
pub enum InternalToolError {
    ...
    #[error("Failed to read symlink {path:?} with ID {id:?} from backend: {source:?}")]
    ReadSymlink {
        path: RepoPath,
        id: SymlinkId,
        source: BackendError,
...
fn read_file_contents(
    ...
    tree: &Tree,
    path: &RepoPath,
) -> Result<FileInfo, InternalToolError> {
    match tree.path_value(path) {
        ...
        Some(TreeValue::File { id, executable }) => {
            let mut reader =
                store
                    .read_file(path, &id)
                    .map_err(|err| InternalToolError::ReadFileBackend {
                        path: path.clone(),
                        id: id.clone(),
                        source: err,
                    })?;
            ...
            reader
                .read_to_end(&mut buf)
                .map_err(|err| InternalToolError::ReadFileIo {
                    path: path.clone(),
                    id: id.clone(),
                    source: err,
                })?;
        ...
        Some(TreeValue::Symlink(id)) => {
            let value =
                store
                    ...
                    .map_err(|err| InternalToolError::ReadSymlink {
                        path: path.clone(),
                        id: id.clone(),
                        source: err,
                    })?;
        ...
        Some(TreeValue::Tree(tree_id)) => {
            unreachable!("list of changed files included a tree: {tree_id:?}");
        }
        Some(TreeValue::GitSubmodule(id)) => Err(InternalToolError::Unimplemented {
            item: "git submodule",
            id: id.hex(),
        }),
        Some(TreeValue::Conflict(id)) => Err(InternalToolError::Unimplemented {
            item: "conflict",
            id: id.hex(),
        }),
    }
...
fn make_diff_sections(
    left_contents: &str,
    right_contents: &str,
) -> Result<Vec<scm_record::Section<'static>>, InternalToolError> {
    ...
    for hunk in diff.hunks() {
        match hunk {
            DiffHunk::Matching(text) => {
                let text =
                    std::str::from_utf8(text).map_err(|err| InternalToolError::DecodeUtf8 {
                        source: err,
            ...
            DiffHunk::Different(sides) => {
                ...
                let left_side =
                    std::str::from_utf8(sides[0]).map_err(|err| InternalToolError::DecodeUtf8 {
                        source: err,
                        item: "left side of diff hunk",
                    })?;
                let right_side =
                    std::str::from_utf8(sides[1]).map_err(|err| InternalToolError::DecodeUtf8 {
                        source: err,
...
) -> Result<Vec<scm_record::File<'static>>, InternalToolError> {
...
pub fn apply_diff_internal(
    ...
    // FIXME: do we need to start from the left or right tree?
    let mut tree_builder = left_tree.store().tree_builder(left_tree.id().clone());
    assert_eq!(
        changed_files.len(),
        files.len(),
    ...
    for (path, file) in changed_files.into_iter().zip(files) {
        ...
        match selected {
            ...
            scm_record::SelectedContents::Binary {
                ...
                // FIXME: use an in-band mechanism to transmit what the binary file was
                // associated with
...
pub fn edit_diff_internal(
    ...
    let result = recorder.run().map_err(InternalToolError::Record)?;
    let tree_id = apply_diff_internal(store, left_tree, right_tree, changed_files, &result.files)
        .map_err(InternalToolError::BackendError)?;
    Ok(tree_id)
}
                ...
                sections.push(match adds.as_slice() {
                    ...
                    _ => {
                        let lines: Vec<scm_record::SectionChangedLine> = adds
                            ...
                            .map(|(contents, change_type)| -> Result<_, InternalToolError> {
...
mod tests {
    ...
    fn test_edit_diff_internal() {
        ...
        let no_changes_tree_id = apply_diff_internal(
            store.clone(),
            &left_tree,
            &right_tree,
            changed_files.clone(),
        ...
        .unwrap();
        let no_changes_tree = store
            .get_tree(&RepoPath::root(), &no_changes_tree_id)
            .unwrap();
        assert_eq!(
            no_changes_tree.id(),
            left_tree.id(),
            "no-changes tree was different: {:?}",
            no_changes_tree.diff_summary(&left_tree, &EverythingMatcher)
        );
        ...
        let all_changes_tree_id = apply_diff_internal(
            store.clone(),
            &left_tree,
            &right_tree,
            changed_files,
        ...
        .unwrap();
        let all_changes_tree = store
            .get_tree(&RepoPath::root(), &all_changes_tree_id)
            .unwrap();
        assert_eq!(
            all_changes_tree.id(),
            right_tree.id(),
            "all-changes tree was different: {:?}",
            all_changes_tree.diff_summary(&right_tree, &EverythingMatcher)
        );
    }
