...
async fn run_inner<'a>(
    ...
    commits: &'static mut [OnDiskCommit],
) -> Result<(), RunError> {
    let mut command_futures = JoinSet::new();
    for commit in commits {
        command_futures.spawn(rewrite_commit(
...
pub fn cmd_run(ui: &mut Ui, command: &CommandHelper, args: &RunArgs) -> Result<(), CommandError> {
    ...
    let shell_command: &'static str = &args.shell_command;
    ...
    let mut stored_commits = create_working_copies(&repo_path, &resolved_commits)?;
    let stored_len = stored_commits.len();

    // Start all the jobs.
    async { run_inner(&tx, sender_tx, jobs, &shell_command, &mut stored_commits).await }
        .block_on()?;
    ...
    tx.mut_repo().transform_descendants(
        ...
        |mut rewriter| {
            ...
            // TODO: how do I rewrite it correctly?
