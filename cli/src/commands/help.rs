...
pub(crate) fn cmd_help(
    ...
    if let [name] = &*args.command {
        if let Some(category) = find_category(name) {
            ui.request_pager();
...
pub fn format_categories() -> String {
    let category_name_max_len = CATEGORIES
        .iter()
        .map(|category| category.name.len())
        .max()
        .unwrap();
    ...
    for category in CATEGORIES {
        write!(
            ret,
            "  {}  ",
            format!("{:<category_name_max_len$}", category.name).bold()
        )
        .unwrap();
