...
fn test_templater_alias() {
    ...
    insta::assert_snapshot!(render_err(r#"if_not_null(label("x", "not boolean"), "")"#), @r###"
    Error: Failed to parse template: Alias "if_not_null()" cannot be expanded
    Caused by:
    1:  --> 1:1
      |
    1 | if_not_null(label("x", "not boolean"), "")
      | ^----------------------------------------^
      |
      = Alias "if_not_null()" cannot be expanded
    2:  --> 1:13
      |
    1 | if_not_null(label("x", "not boolean"), "")
      |             ^-----------------------^
