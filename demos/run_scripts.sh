...
# `convert`. `convert` supports different backends, but the simplest one with no
# dependencies seemed to work for me. Another alternative for SVG -> PNG
# conversion is Inkscape, which can be used directly or via `convert`.
