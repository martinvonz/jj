...
pub fn get_annotation_for_file(
    ...
    let current_tree = starting_commit.tree()?;
    let original_file_id = get_file_id(starting_commit.id(), &current_tree, file_path)?;
    ...
    let revset = RevsetExpression::commit(starting_commit_id.clone())
        ...
        .map_err(|e| match e {
            ...
            RevsetEvaluationError::Other(_) => {
                AnnotateError::FileNotFound(file_name.to_internal_dir_string())
            }
        })?;
...
fn process_file_ids(
    ...
    let current_contents = results.file_cache.get(current_commit_id).unwrap();
    let parent_contents = results.file_cache.get(parent_commit_id).unwrap();
...
fn get_file_id(
    ...
    let file_value = tree
        ...
        .into_resolved()
        .map_err(|_| {
            AnnotateError::Conflicted(file_name.to_internal_dir_string(), commit_id.hex())
        })?
        .unwrap();
