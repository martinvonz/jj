...
fn test_root(use_git: bool) {
    ...
    let new_tree_id = locked_wc.snapshot(GitIgnoreFile::empty(), false).unwrap();
    locked_wc.discard();
...
fn test_gitignores(use_git: bool) {
    ...
    let wc = test_workspace.workspace.working_copy_mut();
    let mut locked_wc = wc.start_mutation();
    let new_tree_id1 = locked_wc.snapshot(GitIgnoreFile::empty(), false).unwrap();
    locked_wc.finish(repo.op_id().clone());
    let tree1 = repo
        .store()
    ...
    let mut locked_wc = wc.start_mutation();
    let new_tree_id2 = locked_wc.snapshot(GitIgnoreFile::empty(), false).unwrap();
    locked_wc.discard();
    let tree2 = repo
        .store()
