...
fn test_update_conflict_from_content_with_long_markers() {
    ...
    let new_left_file_id = testutils::write_file(
        store,
        path,
        indoc! {"
            <<<< left 1
    ...
    let new_right_file_id = testutils::write_file(
        store,
        path,
        indoc! {"
            >>>>>>> right 1
...
fn test_update_from_content_malformed_conflict() {
    ...
    // markers as text
    let new_conflict = parse(&conflict, new_conflict_contents.as_bytes());
    let new_base_file_id = testutils::write_file(
        store,
        path,
        indoc! {"
            line 1
